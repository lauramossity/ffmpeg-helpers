...
def export_video(args):
    ...
    if args.vfs:
        ...
        # Find interlaced frames, deinterlace only interlaced frames, denoise, remove 8px from left and bottom, add
        # black padding back, ensure original scale
        ffmpegArgs.extend(
...
def init_video_clip_subparser(subparsers, name, function, helpMessage, defaultFilterScript, defaultPresetfile):
    ...
    ioGroup.add_argument("-d", "--destination", metavar="DEST", help="path to the destination video file",
                         required=True)
    ...
    timestampGroup = subparser.add_argument_group("Extract segment",
                                                  "See https://trac.ffmpeg.org/wiki/Seeking for details")
    timestampMutexGroup = timestampGroup.add_mutually_exclusive_group()
    timestampMutexGroup.add_argument("--time-segment-fast", nargs=2, metavar=("START_TIMESTAMP", "TIME_LENGTH"),
                                     help="Time segment to extract using the keyframe method. May be inaccurate on "
                                          "stream copy. Example: 1:01.123 2")
    timestampMutexGroup.add_argument("--time-segment-slow", nargs=2, metavar=("START_TIMESTAMP", "END_TIMESTAMP"),
                                     help="Time segment to extract using a slow seek. Example: 1:01.123 1:03:123")
...
def main(args):
    ...
    init_video_clip_subparser(subparsers, 'export', export_video,
                              "Convert a video or video segment to a new format",
                              DEFAULT_FILTER_SCRIPT, DEFAULT_PRESET_FILE)

    # TODO destination should be gif location? or just autogenerate file name
    init_video_clip_subparser(subparsers, 'gif', create_gif,
                              "Export a video clip as a gif",
                              "deinterlace-small.filter", "lm-camcorder-small.ffpreset")
